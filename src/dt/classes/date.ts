...
class bbnDateTool {
  ...
  static compare(date1: any, date2: any, unit: string = ''): -1 | 0 | 1 {
    const d1: any = date1 instanceof bbnDateTool ? date1 : new bbnDateTool(date1);
    const d2: any = date2 instanceof bbnDateTool ? date2 : new bbnDateTool(date2);
    ...
    for (const u of order) {
      const key = getRow(units, (un: string) => un[0] === u)?.[1];  
