...
export default function guessFormat(
  ...
  const tryFormats = (formatsToTry: string[]): string | null => {
  ...
  buildLocaleFromIntl();
  const loc = (bbn as any)?.dt?.locales || {};
  const monthsLong: string[] = loc.monthsLong || [];
  const monthsShort: string[] = loc.monthsShort || [];
  const weekdaysLong: string[] = loc.weekdaysLong || [];
  const weekdaysShort: string[] = loc.weekdaysShort || [];
  const timeFormats: any[] = loc.time || [];
  const dateFormats: any[] = loc.date || [];
  const datetimeFormats: any[] = loc.datetime || [];

  const hasMonthName =
    monthsLong.some(m => lower.includes(m.toLowerCase())) ||
    monthsShort.some(m => lower.includes(m.toLowerCase()));

  const hasWeekdayName =
    weekdaysLong.some(w => lower.includes(w.toLowerCase())) ||
    weekdaysShort.some(w => lower.includes(w.toLowerCase()));

  const hasLetterTZ =
    /gmt|utc|[+-]\d{2}:?\d{2}|z$/i.test(str);

  const looksISO =
    /^\d{4}-\d{2}-\d{2}t\d{2}:\d{2}:\d{2}(\.\d+)?(z|[+\-]\d{2}:?\d{2})?$/i.test(str);

  const looksMySQLDateTime =
    /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}(:\d{2}(\.\d+)?)?$/i.test(str);

  const looksMySQLDate =
    /^\d{4}-\d{2}-\d{2}$/.test(str);

  const looksTimeOnly =
    /^\d{2}:\d{2}(:\d{2}(\.\d+)?)?$/.test(str);
  ...
  if (hasMonthName || hasWeekdayName) {
    ...
    candidates.push(
      'dddd, DD MMMM YYYY HH:II:SSZ',
      'dddd, DD MMMM YYYY HH:II:SS',
      'dddd, DD MMMM YYYY',
      'DD MMMM YYYY HH:II:SSZ',
      'DD MMMM YYYY HH:II:SS',
      'DD MMMM YYYY',
      'ddd, DD MMM YYYY HH:II:SSZ',
      'ddd, DD MMM YYYY HH:II:SS',
      'ddd, DD MMM YYYY'
    );
    ...
    candidates.push(
      'ddd MMM DD YYYY HH:II:SSZ',
      'ddd, DD MMM YYYY HH:II:SS z'
    );
  }
  ...
  candidates.push(
    ...dateFormats.map(f => f.pattern),
    // European style
    'DD/MM/YYYY HH:II:SSZ',
    'DD/MM/YYYY HH:II:SS',
    'DD/MM/YYYY HH:II',
    'DD/MM/YYYY',
